...
func StringLiteral(lcg *common.LCG, hint string) string {
	// Value range for UNIQUE columns to minimize collision probability
	const uniqueValueRange = 1000000

	// Check if this column likely has a UNIQUE constraint based on its name
	// Use word boundaries or exact matches to avoid false positives like "monkey" or "turkey"
	hintLower := strings.ToLower(hint)
	isLikelyUnique := hintLower == "email" ||
		hintLower == "key" ||
		hintLower == "username" ||
		hintLower == "user_name" ||
		strings.HasSuffix(hintLower, "_email") ||
		strings.HasSuffix(hintLower, "_key") ||
	...
	if isLikelyUnique && (choice == 1 || choice == 2) {
		// Re-roll in range [0,12], then map to [0, 3-14] to skip cases 1 and 2
		choice = lcg.Intn(13)
		if choice >= 1 {
	...
	case 8:
		...
		if isLikelyUnique {
			return fmt.Sprintf("'  spaces  %d  '", lcg.Uint64()%uniqueValueRange)
		}
	...
	case 13:
		...
		specials := []string{"!@#$%", "&*()", "<>?", "[]{}"}
		if isLikelyUnique {
			return fmt.Sprintf("'test%s%d'", specials[lcg.Intn(len(specials))], lcg.Uint64()%uniqueValueRange)
		}
	...
	default:
		...
		if isLikelyUnique {
			return fmt.Sprintf(unicodeStrings[lcg.Intn(len(unicodeStrings))], lcg.Uint64()%uniqueValueRange)
		}
