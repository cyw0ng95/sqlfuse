...
// CreateIndexGenerator is a stmts.StmtGenerator for CREATE INDEX statements.
type CreateIndexGenerator struct{}

// Generate implements stmts.StmtGenerator for CREATE INDEX statements.
func (g *CreateIndexGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCreateIndex(ctx.DB, ctx.LCG)
}

// CanGenerate implements stmts.StmtGenerator. CREATE INDEX can always be generated.
func (g *CreateIndexGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}

// DropIndexGenerator is a stmts.StmtGenerator for DROP INDEX statements.
type DropIndexGenerator struct{}

// Generate implements stmts.StmtGenerator for DROP INDEX statements.
func (g *DropIndexGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenDropIndex(ctx.LCG)
}

// CanGenerate implements stmts.StmtGenerator. DROP INDEX can always be generated.
func (g *DropIndexGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func (s *CreateIndexStmt) Flavor() stmts.FlavorConfig { return s.flavor }
...
func (s *DropIndexStmt) Flavor() stmts.FlavorConfig { return s.flavor }
...
func GenCreateIndex(db *sql.DB, lcg *common.LCG) (stmts.Stmt, error) {
	if lcg == nil {
		lcg = common.NewLCG(1)
	}
	...
	for len(indexCols) < nCols {
		...
		colSpec := stmts.QuoteIdent(col.Name)
	...
	sql := fmt.Sprintf("CREATE %sINDEX %s%s ON %s (%s);",
		unique, ifNotExists, stmts.QuoteIdent(indexName), stmts.QuoteIdent(tbl.Name), strings.Join(indexCols, ", "))
	return &CreateIndexStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
}
...
func genCreateIndexFallback(lcg *common.LCG) *CreateIndexStmt {
	...
	return &CreateIndexStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}
}

// GenDropIndex generates a DROP INDEX statement.
// According to Turso COMPAT.md: Partial support (disabled by default).
func GenDropIndex(lcg *common.LCG) (stmts.Stmt, error) {
	if lcg == nil {
		lcg = common.NewLCG(1)
	}
	...
	return &DropIndexStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
}
