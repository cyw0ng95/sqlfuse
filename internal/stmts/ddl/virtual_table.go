...
// CreateVirtualTableGenerator is a stmts.StmtGenerator for CREATE VIRTUAL TABLE statements.
type CreateVirtualTableGenerator struct{}

// Generate implements stmts.StmtGenerator for CREATE VIRTUAL TABLE statements.
func (g *CreateVirtualTableGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCreateVirtualTable(ctx.LCG)
}

// CanGenerate implements stmts.StmtGenerator. CREATE VIRTUAL TABLE can always be generated.
func (g *CreateVirtualTableGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func (s *CreateVirtualTableStmt) Flavor() stmts.FlavorConfig { return s.flavor }
...
func GenCreateVirtualTable(lcg *common.LCG) (stmts.Stmt, error) {
	if lcg == nil {
		lcg = common.NewLCG(1)
	}
	...
	return &CreateVirtualTableStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
}
