...
// CompoundSelectGenerator is a stmts.StmtGenerator for compound SELECT statements (UNION, INTERSECT, EXCEPT).
type CompoundSelectGenerator struct {
	variant stmts.StmtType
}

// Generate implements stmts.StmtGenerator for compound SELECT statements.
func (g *CompoundSelectGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCompoundSelect(ctx.DB, ctx.LCG, g.variant, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. Compound SELECT can always be generated.
func (g *CompoundSelectGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenCompoundSelect(db *sql.DB, lcg *common.LCG, variant stmts.StmtType, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	...
	case stmts.StmtSelectIntersect:
		operator = "INTERSECT"
		stmtType = "select_intersect"
	case stmts.StmtSelectExcept:
		operator = "EXCEPT"
	...
	return &CompoundSelectStmt{
		BaseStmt: stmts.NewBaseStmt(sql, stmtType, flavor),
	}, nil
}

// GenSelectUnion generates a UNION or UNION ALL statement.
func GenSelectUnion(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	return GenCompoundSelect(db, lcg, stmts.StmtSelectUnion, flavor)
}

// GenSelectIntersect generates an INTERSECT statement.
func GenSelectIntersect(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	return GenCompoundSelect(db, lcg, stmts.StmtSelectIntersect, flavor)
}

// GenSelectExcept generates an EXCEPT statement.
func GenSelectExcept(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	return GenCompoundSelect(db, lcg, stmts.StmtSelectExcept, flavor)
}
