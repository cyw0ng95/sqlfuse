...
// UpdateGenerator is a stmts.StmtGenerator for UPDATE statements.
type UpdateGenerator struct{}

// Generate implements stmts.StmtGenerator for UPDATE statements.
func (g *UpdateGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return genUpdateWithFlavor(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. UPDATE can always be generated (creates synthetic tables).
func (g *UpdateGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func (s *UpdateStmt) Flavor() stmts.FlavorConfig { return s.flavor }
...
func GenUpdate(db *sql.DB, lcg *common.LCG) (stmts.Stmt, error) {
	return genUpdateWithFlavor(db, lcg, stmts.GetDefaultFlavor())
}

// genUpdateWithFlavor generates an UPDATE statement with flavor support.
func genUpdateWithFlavor(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	if lcg == nil {
		lcg = common.NewLCG(1)
	}
	if flavor == nil {
	...
	for len(setExprs) < nCols {
		...
		case 0:
			...
			expr = fmt.Sprintf("%s = %s", stmts.QuoteIdent(col.Name), val)
		case 1:
			...
			if isNumericType(col.Type) {
				...
				ops := []string{"+", "-", "*", "/"}
				op := ops[rnd(len(ops))]
				expr = fmt.Sprintf("%s = %s %s %s", stmts.QuoteIdent(col.Name), stmts.QuoteIdent(col.Name), op, val)
			} else {
				val := types.ValueForType(col.Type, lcg, col.Name)
				expr = fmt.Sprintf("%s = %s", stmts.QuoteIdent(col.Name), val)
			}
		case 2:
			...
			expr = fmt.Sprintf("%s = CASE WHEN %s IS NULL THEN %s ELSE %s END",
				stmts.QuoteIdent(col.Name), stmts.QuoteIdent(col.Name), val1, val2)
		case 3:
			// COALESCE expression
			val := types.ValueForType(col.Type, lcg, col.Name)
			expr = fmt.Sprintf("%s = COALESCE(%s, %s)", stmts.QuoteIdent(col.Name), stmts.QuoteIdent(col.Name), val)
		case 4:
			// CAST expression
			targetTypes := []string{"TEXT", "INTEGER", "REAL"}
			targetType := targetTypes[rnd(len(targetTypes))]
			val := types.ValueForType(col.Type, lcg, col.Name)
			expr = fmt.Sprintf("%s = CAST(%s AS %s)", stmts.QuoteIdent(col.Name), val, targetType)
		case 5:
			// NULL assignment
			expr = fmt.Sprintf("%s = NULL", stmts.QuoteIdent(col.Name))
		case 6:
			// String concatenation (for text columns)
			if isTextType(col.Type) {
				val := types.ValueForType(col.Type, lcg, col.Name)
				expr = fmt.Sprintf("%s = %s || %s", stmts.QuoteIdent(col.Name), stmts.QuoteIdent(col.Name), val)
			} else {
				val := types.ValueForType(col.Type, lcg, col.Name)
				expr = fmt.Sprintf("%s = %s", stmts.QuoteIdent(col.Name), val)
			}
		case 7:
			...
			if isTextType(col.Type) {
				...
				expr = fmt.Sprintf("%s = %s(%s)", stmts.QuoteIdent(col.Name), fn, stmts.QuoteIdent(col.Name))
			} else {
				val := types.ValueForType(col.Type, lcg, col.Name)
				expr = fmt.Sprintf("%s = %s", stmts.QuoteIdent(col.Name), val)
			}
	...
	if rnd(4) > 0 { // 75% chance of WHERE clause
		...
		for i := 0; i < numConditions && i < len(tbl.Cols); i++ {
			...
			case 6:
				...
				if isNumericType(col.Type) {
					val := types.ValueForType(col.Type, lcg, col.Name)
					cond = fmt.Sprintf("%s > %s * 2", stmts.QuoteIdent(col.Name), val)
				} else {
					val := types.ValueForType(col.Type, lcg, col.Name)
					cond = fmt.Sprintf("%s = %s", stmts.QuoteIdent(col.Name), val)
				}
	...
	sql := fmt.Sprintf("UPDATE %s SET %s%s;", stmts.QuoteIdent(tbl.Name), strings.Join(setExprs, ", "), where)
...
func genUpdateFallback(lcg *common.LCG) *UpdateStmt {
	return genUpdateFallbackWithFlavor(lcg, stmts.GetDefaultFlavor())
}

// genUpdateFallbackWithFlavor generates a simple UPDATE without schema information with flavor support
func genUpdateFallbackWithFlavor(lcg *common.LCG, flavor stmts.FlavorConfig) *UpdateStmt {
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
