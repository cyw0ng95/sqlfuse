...
func genCoalesceFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	return fmt.Sprintf("coalesce(%s, %s)", stmts.QuoteIdent(col.Name), defaultVal)
}
...
func genConcatFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	for i := 0; i < numArgs; i++ {
		if len(tbls) > 0 && lcg.Intn(2) == 0 {
			...
			if len(tbl.Cols) > 0 {
				col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
				args[i] = stmts.QuoteIdent(col.Name)
				continue
			}
		}
...
func genIfnullFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
	defaultVal := types.ValueForType(col.Type, lcg, col.Name)
	return fmt.Sprintf("ifnull(%s, %s)", stmts.QuoteIdent(col.Name), defaultVal)
}
...
func genTypeofFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	if len(tbl.Cols) > 0 {
		col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
		return fmt.Sprintf("typeof(%s)", stmts.QuoteIdent(col.Name))
	}
	return "typeof(123)"
}
...
func genLikelihoodFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	if len(tbls) > 0 && lcg.Intn(2) == 0 {
		...
		if len(tbl.Cols) > 0 {
			col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
			return fmt.Sprintf("likelihood(%s, %s)", stmts.QuoteIdent(col.Name), prob)
		}
	}
...
func genLikelyFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	if len(tbls) > 0 && lcg.Intn(2) == 0 {
		...
		if len(tbl.Cols) > 0 {
			col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
			return fmt.Sprintf("likely(%s)", stmts.QuoteIdent(col.Name))
		}
	}
	return "likely(1)"
}
...
func genUnlikelyFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	if len(tbls) > 0 && lcg.Intn(2) == 0 {
		...
		if len(tbl.Cols) > 0 {
			col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
			return fmt.Sprintf("unlikely(%s)", stmts.QuoteIdent(col.Name))
		}
	}
	return "unlikely(0)"
}
...
func genSqliteOffsetFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	if len(tbls) > 0 && len(tbls[0].Cols) > 0 {
		...
		return fmt.Sprintf("sqlite_offset(%s)", stmts.QuoteIdent(col.Name))
	}
...
func GenSelectWithAggregateFunction(db *sql.DB, lcg *common.LCG) (SelectStmt, error) {
	...
	sql := fmt.Sprintf("SELECT %s FROM %s;", funcExpr, stmts.QuoteIdent(tbl.Name))
	return SelectStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
}
...
func genCountFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
	return fmt.Sprintf("count(%s)", stmts.QuoteIdent(col.Name))
}
...
func genJSONGroupArrayFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	col := tbl.Cols[lcg.Intn(len(tbl.Cols))]
	return fmt.Sprintf("json_group_array(%s)", stmts.QuoteIdent(col.Name))
}
...
func genJSONGroupObjectFunction(lcg *common.LCG, tbls []helper.TableInfo) string {
	...
	if len(tbl.Cols) < 2 {
		...
		if len(tbl.Cols) == 1 {
			col := tbl.Cols[0]
			return fmt.Sprintf("json_group_object(%s, %s)", stmts.QuoteIdent(col.Name), stmts.QuoteIdent(col.Name))
		}
		return "json_group_object('key', 'value')"
	}
	...
	return fmt.Sprintf("json_group_object(%s, %s)", stmts.QuoteIdent(keyCol.Name), stmts.QuoteIdent(valueCol.Name))
}
	...
	for _, tbl := range tbls {
		for _, col := range tbl.Cols {
			if isNumericType(col.Type) {
				numericCols = append(numericCols, stmts.QuoteIdent(col.Name))
			}
		}
	}
	...
	for _, tbl := range tbls {
		for _, col := range tbl.Cols {
			if isTextType(col.Type) {
				textCols = append(textCols, stmts.QuoteIdent(col.Name))
			}
		}
	}
	if len(textCols) == 0 {
	...
	for _, tbl := range tbls {
		for _, col := range tbl.Cols {
			if isBlobType(col.Type) {
				blobCols = append(blobCols, stmts.QuoteIdent(col.Name))
			}
		}
	}
	if len(blobCols) == 0 {
...
func findAnyColumn(tbls []helper.TableInfo, lcg *common.LCG) string {
	...
	for _, tbl := range tbls {
		for _, col := range tbl.Cols {
			allCols = append(allCols, stmts.QuoteIdent(col.Name))
		}
	}
	if len(allCols) == 0 {
		return ""
