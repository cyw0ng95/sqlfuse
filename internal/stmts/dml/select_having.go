...
func GenSelectHaving(db *sql.DB, lcg *common.LCG) (SelectStmt, error) {
	...
	sql := fmt.Sprintf("SELECT %s, COUNT(1) as cnt FROM %s GROUP BY %s HAVING cnt > %d;", stmts.QuoteIdent(grp.Name), stmts.QuoteIdent(tbl.Name), stmts.QuoteIdent(grp.Name), 1+rnd(10))
