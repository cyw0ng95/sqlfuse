...
func GenSelectWithCTE(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (SelectStmt, error) {
	...
	for len(cteSelectCols) < numCteCols {
		...
		selected[idx] = struct{}{}
		cteSelectCols = append(cteSelectCols, stmts.QuoteIdent(tbl.Cols[idx].Name))
	}
	...
	if rnd(2) == 0 && len(tbl.Cols) > 0 {
		...
		cteWhere = fmt.Sprintf(" WHERE %s > %s", stmts.QuoteIdent(col.Name), val)
	}

	cteQuery := fmt.Sprintf("SELECT %s FROM %s%s",
		strings.Join(cteSelectCols, ", "), stmts.QuoteIdent(tbl.Name), cteWhere)
...
func GenSelectWithMultipleCTE(db *sql.DB, lcg *common.LCG) (SelectStmt, error) {
	...
	for i := 0; i < numCols1 && i < len(tbl1.Cols); i++ {
		cols1 = append(cols1, stmts.QuoteIdent(tbl1.Cols[i].Name))
	}

	cte1Query := fmt.Sprintf("SELECT %s FROM %s LIMIT %d",
		strings.Join(cols1, ", "), stmts.QuoteIdent(tbl1.Name), 10+rnd(40))

	// Second CTE - can reference first CTE or another table
	var cte2Query string
	if rnd(2) == 0 {
		// Reference first CTE
		cte2Query = fmt.Sprintf("SELECT * FROM %s WHERE ROWID %% 2 = 0", stmts.QuoteIdent(cte1Name))
	} else {
		// Use another table
		...
		if len(tbl2.Cols) > 0 {
			...
			for i := 0; i < numCols2 && i < len(tbl2.Cols); i++ {
				cols2 = append(cols2, stmts.QuoteIdent(tbl2.Cols[i].Name))
			}
			cte2Query = fmt.Sprintf("SELECT %s FROM %s LIMIT %d",
				strings.Join(cols2, ", "), stmts.QuoteIdent(tbl2.Name), 5+rnd(20))
		} else {
	...
	case 0:
		...
		mainQuery = fmt.Sprintf("SELECT * FROM %s", stmts.QuoteIdent(cte1Name))
	case 1:
		// SELECT from second CTE only
		mainQuery = fmt.Sprintf("SELECT * FROM %s", stmts.QuoteIdent(cte2Name))
	default:
		// UNION both CTEs
		mainQuery = fmt.Sprintf("SELECT * FROM %s UNION SELECT * FROM %s",
			stmts.QuoteIdent(cte1Name), stmts.QuoteIdent(cte2Name))
	}

	limit := 1 + rnd(50)
	sql := fmt.Sprintf("WITH %s AS (%s), %s AS (%s) %s LIMIT %d;",
		stmts.QuoteIdent(cte1Name), cte1Query, stmts.QuoteIdent(cte2Name), cte2Query, mainQuery, limit)
...
// genSelectWithRecursiveCTEInternal is the internal implementation that uses stmts.GenContext
func genSelectWithRecursiveCTEInternal(ctx *stmts.GenContext) (SelectStmt, error) {
	rnd := ctx.Intn
	...
	if !ctx.SupportsFeature("cte_recursive") {
		...
		sql := fmt.Sprintf("WITH %s AS (SELECT 1 AS n UNION ALL SELECT 2 UNION ALL SELECT 3) SELECT * FROM %s;",
			stmts.QuoteIdent(cteName), stmts.QuoteIdent(cteName))
		return SelectStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
	}
	...
	recursivePatterns := []string{
		...
		fmt.Sprintf("WITH RECURSIVE %s(n) AS (SELECT 1 UNION ALL SELECT n+1 FROM %s WHERE n < %d) SELECT * FROM %s;",
			stmts.QuoteIdent(cteName), stmts.QuoteIdent(cteName), 5+rnd(15), stmts.QuoteIdent(cteName)),

		// Fibonacci-like sequence
		fmt.Sprintf("WITH RECURSIVE %s(a, b) AS (SELECT 0, 1 UNION ALL SELECT b, a+b FROM %s WHERE b < %d) SELECT * FROM %s;",
			stmts.QuoteIdent(cteName), stmts.QuoteIdent(cteName), 100+rnd(900), stmts.QuoteIdent(cteName)),

		// Powers of 2
		fmt.Sprintf("WITH RECURSIVE %s(n, val) AS (SELECT 1, 1 UNION ALL SELECT n+1, val*2 FROM %s WHERE n < %d) SELECT * FROM %s;",
			stmts.QuoteIdent(cteName), stmts.QuoteIdent(cteName), 5+rnd(10), stmts.QuoteIdent(cteName)),
	}
