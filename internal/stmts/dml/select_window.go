...
func GenSelectWithWindowFunction(db *sql.DB, lcg *common.LCG) (SelectStmt, error) {
	...
	ctx := stmts.NewGenContext(db, lcg, 0)
	return genSelectWithWindowFunctionInternal(ctx)
}

// genSelectWithWindowFunctionInternal is the internal implementation that uses stmts.GenContext
func genSelectWithWindowFunctionInternal(ctx *stmts.GenContext) (SelectStmt, error) {
	...
	if !ctx.SupportsFeature("window_functions") {
		...
		if ctx.DB == nil {
			return SelectStmt{sql: "SELECT 1 AS id, 1 AS row_num;", flavor: stmts.GetDefaultFlavor()}, nil
		}
		...
		for len(selectedCols) < numCols {
			...
			selected[idx] = struct{}{}
			selectedCols = append(selectedCols, stmts.QuoteIdent(tbl.Cols[idx].Name))
		}
		...
		sql := fmt.Sprintf("SELECT %s FROM %s LIMIT %d;",
			strings.Join(selectedCols, ", "), stmts.QuoteIdent(tbl.Name), limit)
	...
	case "LAG", "LEAD":
		...
		if len(tbl.Cols) > 0 {
			...
			windowExpr = fmt.Sprintf("%s(%s, %d)", funcName, stmts.QuoteIdent(col.Name), offset)
		} else {
	...
	case "FIRST_VALUE", "LAST_VALUE":
		...
		if len(tbl.Cols) > 0 {
			col := tbl.Cols[rnd(len(tbl.Cols))]
			windowExpr = fmt.Sprintf("%s(%s)", funcName, stmts.QuoteIdent(col.Name))
		} else {
	...
	case "NTH_VALUE":
		...
		if len(tbl.Cols) > 0 {
			col := tbl.Cols[rnd(len(tbl.Cols))]
			n := 1 + rnd(5) // 1-5
			windowExpr = fmt.Sprintf("NTH_VALUE(%s, %d)", stmts.QuoteIdent(col.Name), n)
		} else {
	...
	case 1:
		...
		if len(tbl.Cols) > 0 {
			...
			overClause = fmt.Sprintf("OVER (ORDER BY %s %s)", stmts.QuoteIdent(col.Name), orderDir)
		} else {
			overClause = "OVER ()"
		}
	case 2:
		...
		if len(tbl.Cols) > 1 {
			col := tbl.Cols[rnd(len(tbl.Cols))]
			overClause = fmt.Sprintf("OVER (PARTITION BY %s)", stmts.QuoteIdent(col.Name))
		} else {
			overClause = "OVER ()"
		}
	default:
		...
		if len(tbl.Cols) > 1 {
			...
			overClause = fmt.Sprintf("OVER (PARTITION BY %s ORDER BY %s %s)",
				stmts.QuoteIdent(partCol.Name), stmts.QuoteIdent(orderCol.Name), orderDir)
		} else {
			overClause = "OVER ()"
		}
	}
	...
	selectedCols = append(selectedCols, fmt.Sprintf("%s %s AS %s", windowExpr, overClause, stmts.QuoteIdent(windowAlias)))
...
func genSelectWindowFunctionLiteral(lcg *common.LCG) SelectStmt {
	...
	return SelectStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}
}
...
// genSelectWithMultipleWindowsInternal is the internal implementation that uses stmts.GenContext
func genSelectWithMultipleWindowsInternal(ctx *stmts.GenContext) (SelectStmt, error) {
	// Check if window functions are supported
	if !ctx.SupportsFeature("window_functions") {
		// For flavors that don't support window functions,
		// generate a simple SELECT with aggregates and GROUP BY instead
		if ctx.DB == nil {
			return SelectStmt{sql: "SELECT 1 AS id, COUNT(*) AS cnt;", flavor: stmts.GetDefaultFlavor()}, nil
		}
		...
		if len(tbl.Cols) > 0 {
			...
			sql := fmt.Sprintf("SELECT %s, COUNT(*) AS cnt FROM %s GROUP BY %s LIMIT %d;",
				stmts.QuoteIdent(groupCol.Name), stmts.QuoteIdent(tbl.Name), stmts.QuoteIdent(groupCol.Name), 1+rnd(30))
			return SelectStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
		}
	}
	...
	for i := 0; i < numWindows; i++ {
		...
		if funcType == "ROW_NUMBER" || funcType == "RANK" {
			...
			if found {
				funcExpr = fmt.Sprintf("%s(%s)", funcType, stmts.QuoteIdent(numCol.Name))
			} else {
		...
		if i == 0 && len(tbl.Cols) > 0 {
			...
			col := tbl.Cols[rnd(len(tbl.Cols))]
			overClause = fmt.Sprintf("OVER (ORDER BY %s)", stmts.QuoteIdent(col.Name))
			...
			overClause = fmt.Sprintf("OVER (PARTITION BY %s ORDER BY %s DESC)",
				stmts.QuoteIdent(partCol.Name), stmts.QuoteIdent(orderCol.Name))
		} else {
			overClause = "OVER ()"
		}

		windowAlias := fmt.Sprintf("win%d", i+1)
		windowExprs = append(windowExprs, fmt.Sprintf("%s %s AS %s", funcExpr, overClause, stmts.QuoteIdent(windowAlias)))
	}

	allCols := append(selectedCols, windowExprs...)
	limit := 1 + rnd(30)
	sql := fmt.Sprintf("SELECT %s FROM %s LIMIT %d;",
		strings.Join(allCols, ", "), stmts.QuoteIdent(tbl.Name), limit)
	return SelectStmt{sql: sql, flavor: stmts.GetDefaultFlavor()}, nil
}
