...
func (g *SelectIndexedByGenerator) Generate(ctx *GenContext) (Stmt, error) {
	return genSelectIndexedBy(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements StmtGenerator
func (g *SelectIndexedByGenerator) CanGenerate(ctx *GenContext) bool {
...
func genSelectIndexedBy(db *sql.DB, lcg interface{ Intn(int) int }, flavor FlavorConfig) (Stmt, error) {
	if flavor == nil {
...
func (g *SelectNotIndexedGenerator) Generate(ctx *GenContext) (Stmt, error) {
	return genSelectNotIndexed(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements StmtGenerator
func (g *SelectNotIndexedGenerator) CanGenerate(ctx *GenContext) bool {
	tables, err := helper.GetAllTablesAndCols(ctx.DB, ctx.Flavor.Name())
	return err == nil && len(tables) > 0
}

// genSelectNotIndexed generates a SELECT with NOT INDEXED clause
func genSelectNotIndexed(db *sql.DB, lcg interface{ Intn(int) int }, flavor FlavorConfig) (Stmt, error) {
	if flavor == nil {
		flavor = GetDefaultFlavor()
	}
