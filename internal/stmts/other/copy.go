...
// CopyGenerator is a stmts.StmtGenerator for COPY statements.
type CopyGenerator struct{}

// Generate implements stmts.StmtGenerator for COPY statements.
func (g *CopyGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCopy(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. COPY requires tables to exist for export.
func (g *CopyGenerator) CanGenerate(ctx *stmts.GenContext) bool {
...
func GenCopy(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	...
	return &CopyStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "copy", flavor),
	}, nil
}
