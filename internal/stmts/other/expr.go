...
func (eg *ExprGenerator) GenCastExpr(tbls []helper.TableInfo) string {
	...
	return fmt.Sprintf("CAST(%s AS %s)", stmts.QuoteIdent(col.Name), targetType)
}
...
func (eg *ExprGenerator) GenBetweenExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	return fmt.Sprintf("%s %sBETWEEN %s AND %s", stmts.QuoteIdent(col.Name), notClause, val1, val2)
}
...
func (eg *ExprGenerator) GenGlobExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	return fmt.Sprintf("%s %sGLOB '%s'", stmts.QuoteIdent(col.Name), notClause, pattern)
}
...
func (eg *ExprGenerator) GenIsDistinctFromExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	return fmt.Sprintf("%s IS %sDISTINCT FROM %s", stmts.QuoteIdent(col.Name), notClause, compareVal)
}
...
func (eg *ExprGenerator) GenCollateExpr(tbls []helper.TableInfo) string {
	...
	return fmt.Sprintf("%s COLLATE %s", stmts.QuoteIdent(col.Name), collation)
}
...
func (eg *ExprGenerator) GenUnaryExpr(tbls []helper.TableInfo) string {
	...
	if op == "NOT" {
		...
		return fmt.Sprintf("NOT (%s = %s)", stmts.QuoteIdent(col.Name), val)
	}

	// +, -, ~ work with numeric values
	if len(numericCols) > 0 {
		col := numericCols[eg.ctx.Intn(len(numericCols))]
		return fmt.Sprintf("%s%s", op, stmts.QuoteIdent(col.Name))
	}
...
func (eg *ExprGenerator) GenBinaryExpr(tbls []helper.TableInfo) string {
	...
	case 0: // Numeric operators
		if len(numericCols) >= 2 {
			...
			return fmt.Sprintf("%s %s %s", stmts.QuoteIdent(col1.Name), op, stmts.QuoteIdent(col2.Name))
		}
		op = numericOps[eg.ctx.Intn(len(numericOps))]
	...
	case 1: // Comparison operators
		...
		return fmt.Sprintf("%s %s %s", stmts.QuoteIdent(col1.Name), op, val)
	...
	default: // Logical operators
		...
		return fmt.Sprintf("(%s = %s) %s (%s = %s)", stmts.QuoteIdent(col1.Name), val1, op, stmts.QuoteIdent(col2.Name), val2)
	}
}
...
func (eg *ExprGenerator) GenParenthesizedExpr(tbls []helper.TableInfo) string {
	...
	return fmt.Sprintf("(%s = %s)", stmts.QuoteIdent(col.Name), val)
}
...
func (eg *ExprGenerator) GenIsNullExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	if useNot {
		return fmt.Sprintf("%s IS NOT NULL", stmts.QuoteIdent(col.Name))
	}
	return fmt.Sprintf("%s IS NULL", stmts.QuoteIdent(col.Name))
}
...
func (eg *ExprGenerator) GenRegexpExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	return fmt.Sprintf("%s %sREGEXP '%s'", stmts.QuoteIdent(col.Name), notClause, pattern)
}
...
func (eg *ExprGenerator) GenMatchExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	return fmt.Sprintf("%s %sMATCH '%s'", stmts.QuoteIdent(col.Name), notClause, pattern)
}
...
func (eg *ExprGenerator) GenInSubqueryExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	subquery := fmt.Sprintf("SELECT %s FROM %s LIMIT 5", stmts.QuoteIdent(col.Name), stmts.QuoteIdent(tbl.Name))
	...
	return fmt.Sprintf("%s %sIN (%s)", stmts.QuoteIdent(col.Name), notClause, subquery)
}
...
func (eg *ExprGenerator) GenExistsSubqueryExpr(tbls []helper.TableInfo, useNot bool) string {
	...
	if len(tbl.Cols) > 0 {
		col := tbl.Cols[eg.ctx.Intn(len(tbl.Cols))]
		val := types.ValueForType(col.Type, eg.ctx.LCG, col.Name)
		subquery = fmt.Sprintf("SELECT 1 FROM %s WHERE %s = %s", stmts.QuoteIdent(tbl.Name), stmts.QuoteIdent(col.Name), val)
	} else {
		subquery = fmt.Sprintf("SELECT 1 FROM %s", stmts.QuoteIdent(tbl.Name))
	}
...
func (eg *ExprGenerator) GenFilterExpr(tbls []helper.TableInfo) string {
	...
	if agg == "COUNT" && eg.ctx.Intn(2) == 0 {
		...
		aggArg = stmts.QuoteIdent(col.Name)
	}
	...
	return fmt.Sprintf("%s(%s) FILTER (WHERE %s = %s)", agg, aggArg, stmts.QuoteIdent(filterCol.Name), filterVal)
}
...
func (eg *ExprGenerator) GenOverExpr(tbls []helper.TableInfo) string {
	...
	if eg.ctx.Intn(2) == 0 && len(tbl.Cols) > 1 {
		...
		overClause = fmt.Sprintf("PARTITION BY %s ORDER BY %s", stmts.QuoteIdent(partCol.Name), stmts.QuoteIdent(col.Name))
	} else {
		overClause = fmt.Sprintf("ORDER BY %s", stmts.QuoteIdent(col.Name))
	}
