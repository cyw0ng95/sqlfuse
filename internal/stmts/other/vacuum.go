...
// VacuumGenerator is a stmts.StmtGenerator for VACUUM statements.
type VacuumGenerator struct{}

// Generate implements stmts.StmtGenerator for VACUUM statements.
func (g *VacuumGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenVacuum(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. VACUUM can always be generated.
func (g *VacuumGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenVacuum(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	...
	return &VacuumStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "vacuum", flavor),
	}, nil
}
