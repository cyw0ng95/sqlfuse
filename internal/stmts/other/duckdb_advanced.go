...
// MergeIntoGenerator is a stmts.StmtGenerator for MERGE INTO statements.
type MergeIntoGenerator struct{}

// Generate implements stmts.StmtGenerator for MERGE INTO statements.
func (g *MergeIntoGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenMergeInto(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. MERGE INTO requires tables.
func (g *MergeIntoGenerator) CanGenerate(ctx *stmts.GenContext) bool {
...
func GenMergeInto(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
...
WHEN NOT MATCHED THEN INSERT (id, value) VALUES (source_table.id, source_table.value);`
	...
	return &MergeIntoStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "merge_into", flavor),
	}, nil
}

// QualifyGenerator is a stmts.StmtGenerator for SELECT with QUALIFY clause.
type QualifyGenerator struct{}

// Generate implements stmts.StmtGenerator for QUALIFY clause.
func (g *QualifyGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenQualify(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. QUALIFY can always be generated.
func (g *QualifyGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenQualify(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
...
QUALIFY NTILE(4) OVER (ORDER BY amount) = 1;`
	...
	return &QualifyStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "select_qualify", flavor),
	}, nil
}

// AlterDatabaseGenerator is a stmts.StmtGenerator for ALTER DATABASE statements.
type AlterDatabaseGenerator struct{}

// Generate implements stmts.StmtGenerator for ALTER DATABASE statements.
func (g *AlterDatabaseGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenAlterDatabase(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. ALTER DATABASE can always be generated.
func (g *AlterDatabaseGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenAlterDatabase(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &AlterDatabaseStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "alter_database", flavor),
	}, nil
}

// AlterViewGenerator is a stmts.StmtGenerator for ALTER VIEW statements.
type AlterViewGenerator struct{}

// Generate implements stmts.StmtGenerator for ALTER VIEW statements.
func (g *AlterViewGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenAlterView(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. ALTER VIEW can always be generated.
func (g *AlterViewGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenAlterView(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &AlterViewStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "alter_view", flavor),
	}, nil
}
