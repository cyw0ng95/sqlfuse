...
// AttachGenerator is a stmts.StmtGenerator for ATTACH DATABASE statements.
type AttachGenerator struct{}

// Generate implements stmts.StmtGenerator for ATTACH DATABASE statements.
func (g *AttachGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenAttachDatabase(ctx.LCG), nil
}

// CanGenerate implements stmts.StmtGenerator. ATTACH DATABASE can always be generated.
func (g *AttachGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}

// DetachGenerator is a stmts.StmtGenerator for DETACH DATABASE statements.
type DetachGenerator struct{}

// Generate implements stmts.StmtGenerator for DETACH DATABASE statements.
func (g *DetachGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenDetachDatabase(ctx.LCG), nil
}

// CanGenerate implements stmts.StmtGenerator. DETACH DATABASE can always be generated.
func (g *DetachGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenAttachDatabase(lcg *common.LCG) stmts.Stmt {
	...
	return &AttachStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "attach", stmts.GetDefaultFlavor()),
	}
}

// GenDetachDatabase generates a DETACH DATABASE statement.
// According to Turso COMPAT.md: Yes (full support).
func GenDetachDatabase(lcg *common.LCG) stmts.Stmt {
	lcg = stmts.EnsureLCG(lcg)
	...
	return &DetachStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "detach", stmts.GetDefaultFlavor()),
	}
}
