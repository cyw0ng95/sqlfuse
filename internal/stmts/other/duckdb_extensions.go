...
// CreateSecretGenerator is a stmts.StmtGenerator for CREATE SECRET statements.
type CreateSecretGenerator struct{}

// Generate implements stmts.StmtGenerator for CREATE SECRET statements.
func (g *CreateSecretGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCreateSecret(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. CREATE SECRET can always be generated.
func (g *CreateSecretGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenCreateSecret(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	...
	return &CreateSecretStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "create_secret", flavor),
	}, nil
}

// DropSecretGenerator is a stmts.StmtGenerator for DROP SECRET statements.
type DropSecretGenerator struct{}

// Generate implements stmts.StmtGenerator for DROP SECRET statements.
func (g *DropSecretGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenDropSecret(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. DROP SECRET can always be generated.
func (g *DropSecretGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenDropSecret(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &DropSecretStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "drop_secret", flavor),
	}, nil
}

// LoadInstallGenerator is a stmts.StmtGenerator for LOAD/INSTALL statements.
type LoadInstallGenerator struct{}

// Generate implements stmts.StmtGenerator for LOAD/INSTALL statements.
func (g *LoadInstallGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenLoadInstall(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. LOAD/INSTALL can always be generated.
func (g *LoadInstallGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenLoadInstall(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &LoadInstallStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "load_install", flavor),
	}, nil
}

// CommentOnGenerator is a stmts.StmtGenerator for COMMENT ON statements.
type CommentOnGenerator struct{}

// Generate implements stmts.StmtGenerator for COMMENT ON statements.
func (g *CommentOnGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenCommentOn(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. COMMENT ON can always be generated.
func (g *CommentOnGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenCommentOn(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &CommentOnStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "comment_on", flavor),
	}, nil
}

// ProfilingGenerator is a stmts.StmtGenerator for profiling statements.
type ProfilingGenerator struct{}

// Generate implements stmts.StmtGenerator for profiling statements.
func (g *ProfilingGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenProfiling(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. Profiling can always be generated.
func (g *ProfilingGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenProfiling(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &ProfilingStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "profiling", flavor),
	}, nil
}

// SetVariableGenerator is a stmts.StmtGenerator for SET VARIABLE statements.
type SetVariableGenerator struct{}

// Generate implements stmts.StmtGenerator for SET VARIABLE statements.
func (g *SetVariableGenerator) Generate(ctx *stmts.GenContext) (stmts.Stmt, error) {
	return GenSetVariable(ctx.DB, ctx.LCG, ctx.Flavor)
}

// CanGenerate implements stmts.StmtGenerator. SET VARIABLE can always be generated.
func (g *SetVariableGenerator) CanGenerate(ctx *stmts.GenContext) bool {
	return true
}
...
func GenSetVariable(db *sql.DB, lcg *common.LCG, flavor stmts.FlavorConfig) (stmts.Stmt, error) {
	lcg = stmts.EnsureLCG(lcg)
	if flavor == nil {
		flavor = stmts.GetDefaultFlavor()
	}
	...
	return &SetVariableStmt{
		BaseStmt: stmts.NewBaseStmt(sql, "set_variable", flavor),
	}, nil
}
